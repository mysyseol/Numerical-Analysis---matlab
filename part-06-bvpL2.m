...
d0=[0,1,0]; d1=[-1/2,0,1/2]/h; d2=[1,-2,1]/h^2;
tri=zeros(n,4);
for i=1:n
    tri(i,1:3)=d2+p(i)*d1+q(i)*d0; tri(i,4)=r(i);
end

% bc at x=a
t=c(1,1)*d1+c(1,2)*d0; t=[t,c(1,3)];
if t(1)==0, tri(1,:)=t;
else tri(1,:)=tri(1,:)-tri(1,1)/t(1)*t; end

% bc at x=b
t=c(2,1)*d1+c(2,2)*d0; t=[t,c(2,3)];
if t(3)==0, tri(n,:)=t;
else tri(n,:)=tri(n,:)-tri(n,3)/t(3)*t; end
